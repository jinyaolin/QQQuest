...
from core.socket_client import SocketClient, read_socket_server_log
from config.constants import DeviceStatus, STATUS_ICONS
from utils.logger import get_logger

from .ui_utils import hide_dialog_close_button
...
def edit_room_dialog(room: Room):
    ...
    if f'editing_param_{room.room_id}' in st.session_state:
        ...
        # 輔助函數：根據類型渲染輸入框
        def render_param_input(label, current_value, key_suffix):
        ...
        if is_global:
            new_global_value = render_param_input("全域值", current_param.global_value, "global")
        else:
            ...
            for dev in room_devices:
                ...
                new_val = render_param_input(f"{dev.display_name} ({dev.ip})", existing_val, f"dev_{dev.device_id}")
...
def room_view_dialog(room: Room):
    ...
    # 使用標籤頁分隔不同狀態的設備
    from .room_card import render_devices_in_room
    tabs_data = []
    if online_devices:
